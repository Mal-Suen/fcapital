...
│                           Core Layer (核心层)                                │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                    ReconOrchestrator (信息收集编排器)                  │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │   │
│  │  │ subfinder│ │  httpx   │ │  dnsx    │ │  nmap    │ │ wafw00f  │   │   │
│  │  │ 子域名   │ │ HTTP探测 │ │ DNS查询  │ │ 端口扫描 │ │ WAF检测  │   │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘   │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐                               │   │
│  │  │ SSL/TLS  │ │敏感文件  │ │技术栈识别│                               │   │
│  │  └──────────┘ └──────────┘ └──────────┘                               │   │
...
| **ReconOrchestrator** | 信息收集编排，并行执行多个收集任务 | Module Layer |
| **AIEngine** | AI 对话管理，决策分析 | AIProvider |
| **ContextManager** | 上下文收集、存储、压缩 | Storage |
| **ToolScheduler** | 工具选择、安装、执行 | ToolRunner, ToolCheck |
| **Dispatcher** | 决策分发，标准/非标准/混合场景判断 | ToolScheduler, ScriptGenerator |
| **ScriptGenerator** | AI脚本生成、安全检查、执行 | AIEngine, SandboxRunner |
| **ToolCheck** | 工具可用性检测 | ToolRunner |
| **ToolInstall** | 工具自动安装 | - |
